  ...
  if (req.url === "/api/credit-package" && req.method === "GET") {
    ...
    req.on("end", async () => {
      try {
        ...
        const creditSkillRepo = await AppDataSource.getRepository("Skill")
        const existSkill = await creditSkillRepo.find({
          where: {
            name: data.name
          }
        })
        ...
        const newSkill = await creditSkillRepo.create({
          name: data.name,
          credit_amount: data.credit_amount,
          price: data.price
        })
        const result = await creditSkillRepo.save(newSkill)
        res.writeHead(200, headers)
        res.write(JSON.stringify({
          status: "success",
          data: result
