...
async function postCourse (req, res, next) {
    try {
      ...
      validCheck.isUndefined(name) || validCheck.isNotValidSting(name) ||
      validCheck.isUndefined(description) || validCheck.isNotValidSting(description) ||
      validCheck.isUndefined(startAt) || validCheck.isNotValidSting(startAt) ||
      validCheck.isUndefined(endAt) || validCheck.isNotValidSting(endAt) ||
      validCheck.isUndefined(maxParticipants) || validCheck.isNotValidInteger(maxParticipants) ||
      validCheck.isUndefined(meetingUrl) || validCheck.isNotValidSting(meetingUrl) || !meetingUrl.startsWith('https')) {
        logger.warn('欄位未填寫正確')
...
async function getCoachRevenue (req, res, next) {
  try {
    ...
    if (validCheck.isUndefined(month) || !Object.prototype.hasOwnProperty.call(monthMap, month)) {
      logger.warn('欄位未填寫正確')
      return next(appError(400, 'failed', '欄位未填寫正確', next))
    }
    const courseRepo = dataSource.getRepository('Course')
...
async function putCoachCourseDetail (req, res, next) {
  try {
    ...
    if (validCheck.isNotValidSting(courseId) ||
    validCheck.isUndefined(skillId) || validCheck.isNotValidSting(skillId) ||
    validCheck.isUndefined(name) || validCheck.isNotValidSting(name) ||
    validCheck.isUndefined(description) || validCheck.isNotValidSting(description) ||
    validCheck.isUndefined(startAt) || validCheck.isNotValidSting(startAt) ||
    validCheck.isUndefined(endAt) || validCheck.isNotValidSting(endAt) ||
...
async function postCoach (req, res, next) {
  try {
    ...
    if (validCheck.isUndefined(experienceYears) || validCheck.isNotValidInteger(experienceYears) || validCheck.isUndefined(description) || validCheck.isNotValidSting(description)) {
      logger.warn('欄位未填寫正確')
      return next(appError(400, 'failed', '欄位未填寫正確', next))
    }
    if (profileImageUrl && !validCheck.isNotValidSting(profileImageUrl) && !profileImageUrl.startsWith('https')) {
      logger.warn('大頭貼網址錯誤')
      return next(appError(400, 'failed', '欄位未填寫正確', next))
    }
    const userRepository = dataSource.getRepository('User')
...
async function putCoachProfile (req, res, next) {
  try {
    ...
    if (validCheck.isUndefined(experienceYears) || validCheck.isNotValidInteger(experienceYears) ||
    validCheck.isUndefined(description) || validCheck.isNotValidSting(description) ||
      validCheck.isUndefined(profileImageUrl) || validCheck.isNotValidSting(profileImageUrl) ||
      !profileImageUrl.startsWith('https') ||
      validCheck.isUndefined(skillIds) || !Array.isArray(skillIds)) {
      logger.warn('欄位未填寫正確')
      return next(appError(400, 'failed', '欄位未填寫正確', next))
    }
    if (skillIds.length === 0 || skillIds.every(skill => validCheck.isUndefined(skill) || validCheck.isNotValidSting(skill))) {
      logger.warn('欄位未填寫正確')
      return next(appError(400, 'failed', '欄位未填寫正確', next))
    }
    const coachRepo = dataSource.getRepository('Coach')
